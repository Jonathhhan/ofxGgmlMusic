param(
	[string] $SmokePath = ".music-runtime-smoke.json",
	[string] $OutputPath = "build\evidence\music-runtime-evidence.json",
	[string] $Repo = "ofxGgmlMusic",
	[string] $Lane = "music-generation",
	[string] $CommitSha = "",
	[string] $WorkflowName = "music-runtime-smoke",
	[string] $RunnerOs = "",
	[string] $Backend = "",
	[string] $ArtifactPath = "",
	[string] $Command = "",
...
function Get-ToolVersions {
	...
	$versions["producer"] = "write-music-runtime-evidence.ps1"
...
if (!(Test-Path -LiteralPath $resolvedSmokePath -PathType Leaf)) {
	throw "Music runtime smoke JSON was not found: $resolvedSmokePath"
}
...
if ($null -eq $summary) {
	throw "Music runtime smoke JSON must include a Summary object. Run scripts\run-music-runtime-smoke.ps1 -Json -SummaryOnly -OutputPath .music-runtime-smoke.json first."
}
...
if ([string]::IsNullOrWhiteSpace($Command)) {
	$Command = "scripts/run-music-runtime-smoke.ps1 -Json -SummaryOnly -OutputPath $ArtifactPath"
}

$result = if ($summary.Passed -eq $true) { "pass" } else { "fail" }
$inferenceChecked = [bool] $summary.InferenceChecked
$certificationLevel = if ($summary.Passed -eq $true -and $inferenceChecked) { "runtime-certified" } else { "declared" }
...
$evidence = [ordered] @{
	...
	producer = "write-music-runtime-evidence.ps1"
...
if ($PassThru) { Write-Output $content } else { Write-Host "Wrote Music runtime Evidence Schema v1 wrapper: $resolvedOutputPath" }
exit 0
