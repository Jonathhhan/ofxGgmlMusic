...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofxGgmlMusic-tests"
}
...
if (Test-WindowsHost) {
	...
	Write-Step "Configuring and running ofxGgmlMusic tests with Visual Studio tools"
	Invoke-CheckedCmd "ofxGgmlMusic tests" $command
} else {
	Write-Step "Configuring ofxGgmlMusic tests"
	Invoke-CheckedNative "cmake configure ofxGgmlMusic tests" {
		cmake -S $testsDir -B $BuildDir -DCMAKE_BUILD_TYPE=$Configuration
	}
	Write-Step "Building ofxGgmlMusic tests"
	Invoke-CheckedNative "cmake build ofxGgmlMusic tests" {
		cmake --build $BuildDir --config $Configuration
	}
	Write-Step "Running ofxGgmlMusic tests"
	Invoke-CheckedNative "ctest ofxGgmlMusic tests" {
