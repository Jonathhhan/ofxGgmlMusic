...
WAVs are written next to the mix. Each Generate press writes a timestamped WAV
instead of overwriting the previous render. On startup, the example reloads the
previous history entry or standard output manifest if it exists; use `Reload` to
refresh it manually. The `Recent` menu reads `ofxGgmlMusic-history.json` from
