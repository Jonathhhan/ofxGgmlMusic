...
bass, and pulse stem WAVs are written next to the mix, and editable melody/chord
`.mid` files are written beside the WAV. Each Generate press writes a timestamped
WAV instead of overwriting the previous render. On startup, the example reloads
the previous history entry or standard output manifest if it exists; use `Reload`
to refresh it manually. The `Recent` menu reads `ofxGgmlMusic-history.json` from
the output directory and can reload manifests generated by the example or CLI.
